...
class Gauss_NN_Norm:
    ...
    inputs:
        ...
        R - parameter used in the Radial basis Fucntion- this will scale all of the RR's to make them all uniformly larger or smaller as needed
        D - Dimension of th system being trained on
        b- the number of nearest neighbors to average over to make RR (the Relative R)
    """
    def FuncApproxF(self,Xdata,length,p,beta,R,D,b):
        #First we perform a scaling down of our data
        DData = Xdata.copy()
        self.Scaler = np.zeros(D)
        for d in range(D):
            self.Scaler[d] = abs(max(Xdata[d])-min(Xdata[d]))
            DData[d] = DData[d]/self.Scaler[d]
        ...
        XdataT = DData.T
        self.D = D
        Ydata = self.GenY(DData,length,D)
